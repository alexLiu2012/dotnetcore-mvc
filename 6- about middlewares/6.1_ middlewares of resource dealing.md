...
##### 8.1.1 spa proxy launch manager
...
internal class SpaProxyLaunchManager : IDisposable
{
    ...
    {
        // 注入 spa development server options
        _options = options.Value;
        _logger = logger;
    }
    ...
    public void StartInBackground(CancellationToken cancellationToken)
    {
        ...
        // We are not waiting for the SPA proxy to launch, instead we are going to rely on a piece of middleware to display 
        // an HTML document while the SPA proxy is not ready, refresh every three seconds and redirect to the SPA proxy url 
        // once it is ready.
        // Being ready in this context means that we were able to receive a 200 from the proxy or that we gave up waiting.
        ...
        CancellationToken cancellationToken)
    {
        ...
        while (_spaProcess != null && 
               !_spaProcess.HasExited && 
               !maxTimeoutReached)
        {
            // 2- probe spa dev server
...
##### 8.1.2 spa development server options
...
##### 8.1.3 spa proxy startup filter

```c#
internal class SpaProxyStartupFilter : IStartupFilter
{
    // 注入 spa proxy middleware
    ...
    public async Task Invoke(HttpContext context)
    {
        ...
        string GenerateSpaLaunchPage(SpaDevelopmentServerOptions options)
        {
            return $@"
	            ...
	            <p>This page will automatically redirect to <a href=""{HtmlEncoder.Default.Encode(options.ServerUrl)}"">
    	        HtmlEncoder.Default.Encode(options.ServerUrl)}</a> when the SPA proxy is ready.</p>
        	    </body>
            	</html>";
        }
    }
...
###### 8.2.1.2 spa builder proxy extensions
...
internal static class SpaProxy
{
        ...
        bool proxy404s)
    {
        ...
        try
        {
            ...
            {                
                // 2- create proxy request message，
                        ...
                        proxyCancellationToken))
                {
                    ...
                    // 3- 将 response message 注入原有 http context
...
##### 8.2.2 spa default page middleware
...
internal class SpaDefaultPageMiddleware
{
    public static void Attach(ISpaBuilder spaBuilder)
    {
        ...
        // 1- 注入中间件，Rewrite all requests to the default page
        app.Use(
            (context, next) =>
                {
        ...
        // 2- 注入 use spa static file 中间件（终结终结点）
        ...
        // 3- 注入中间件（如果没有找 static file，-> 抛出异常）
        app.Use(
            (context, next) =>
                {
                    // 如果有 endpoint，-> 下一个中间件
...
internal class NodeScriptRunner : IDisposable
{
    ...
    public void AttachToLogger(ILogger logger)
    {
        ...
        // But when it emits incomplete lines, assume this is progress information and hence just 
        // pass it through to StdOut regardless of logger config.
...
{            
        ...
        string npmScript)
    {
        ...
        // 如果 spa options 的 spa server source path 为空，-> 抛出异常
...
###### 8.2.4.1 angular cli middleware
...
internal static class AngularCliMiddleware
{
    ...
    {
        // 从 spa options 解析 cmd、source path 和 port
...
###### 8.2.5.1 react server middleware
...
internal static class ReactDevelopmentServerMiddleware
{
    ...
    public static void Attach(
        ...
        string scriptName)
    {
        // 从 spa options 解析 cmd、server source path 和 port
        var pkgManagerCommand = spaBuilder.Options.PackageManagerCommand;
        var sourcePath = spaBuilder.Options.SourcePath;
        var devServerPort = spaBuilder.Options.DevServerPort;
        ...
        // - the requests are always from the local machine (we're not accepting remote requests that go directly to 
        //	 the create-react-app server)
        // - given that, there's no reason to use https, and we couldn't even if we wanted to, because in general 
        //	 the create-react-app server has no certificate
